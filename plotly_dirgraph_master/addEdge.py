...
def addEdge_3D(start, end, edge_x, edge_y, edge_z, lengthFrac=1, arrowPos = None, arrowLength=0.025, arrowAngle = 30, dotSize=20):
    ...
    if not arrowPos == None:
        ...
        signx = (x1-x0)/abs(x1-x0) if x1!=x0 else +1    #verify this once
        signy = (y1-y0)/abs(y1-y0) if y1!=y0 else +1    #verified
        signz = (z1-z0)/abs(z1-z0) if z1!=z0 else +1

        # Append first arrowhead
        dx = arrowLength * math.sin(math.radians(eta + arrowAngle))
        dy = arrowLength * math.cos(math.radians(eta + arrowAngle))
        dz = arrowLength * math.cos(math.radians(eta + arrowAngle))
        edge_x.append(pointx)
        edge_x.append(pointx - signx**2 * signy * dx)
        edge_x.append(None)
        edge_y.append(pointy)
        edge_y.append(pointy - signx**2 * signy * dy)
        edge_y.append(None)
        edge_z.append(pointz)
        edge_z.append(pointz - signx**1 * signy * dz)
        ...
        edge_y.append(pointy - signx**2 * signy * dy)
        edge_y.append(None)
        edge_y.append(pointz)
        edge_y.append(pointz - signx**2 * signy * dz)
        edge_y.append(None)
